...
function check_for_wizard_update($echo_message = false)
{
    ...
    if ($update_available) {
        if ($echo_message) {
            echo '<p class="alert">Обновленная версия этого скрипта мастера доступена <a href="' . LOADER_WIZARD_URL . '">here</a>.</p>';
        }
...
function server_restriction_warnings()
{
    ...
    if (find_server_type() == SERVER_SHARED) {
        if (is_restricted_server()) {
            $warnings[] = "Ограничения сервера находятся в месте, которое может повлиять на работу данного мастера Loader или предотвратить установку Loader.";
        }
    } else {
        $warning_suffix = "Это может повлиять на работу данного мастера Loader.";
        if (ini_get('safe_mode')) {
            $warnings[] = "Безопасный режим действует на сервере. " . $warning_suffix;
        } 
        $disabled_functions = ini_get('disable_functions');
        if (!empty($disabled_functions)) {
            $warnings[] = "Некоторые функции отключены через disable_functions. " . $warning_suffix;
        }
        $open_basedir = ini_get('open_basedir');
        if (!empty($open_basedir)) {
            $warnings[] = "Открытые ограничения BaseDir в действительности. " . $warning_suffix;
        }
    }
    return $warnings;
}
...
function get_loader_location_from_ini($php_ini = '')
{
    ...
    if (!@file_exists($php_ini)) {
        if (empty($php_ini)) {
            $errors[ERROR_INI_NOT_FOUND] = "Файл конфигурации не может быть найден.";
        } else {
            $errors[ERROR_INI_NOT_FOUND] = "Файл $php_ini не найден";
        }
    } elseif (!is_readable($php_ini)) {
        $errors[ERROR_INI_NOT_READABLE] = "Файл $php_ini не прочитан";
    }
    if (!empty($errors)) {
        ...
        if (preg_match("/^\s*$ext_start\s*=\s*\"?([^\"]+)\"?/i",$l,$corr_matches)) {
            if (preg_match("/$loader_name_match/i",$corr_matches[1])) {
                if (!empty($loader_path)) {
                    $errors[ERROR_INI_MULTIPLE_IC_LOADER_LINES] = "Похоже, что несколько $ext_start линий для IonCube Loader были включены в конфигурационном файле, $php_ini.";
                }
                $loader_path = $corr_matches[1];
            } else {
                if (empty($loader_path)) {
                    $errors[ERROR_INI_NOT_FIRST_ZE] = "IonCube погрузчик должен быть первым расширением Zend, перечисленным в конфигурационном файле, $php_ini.";
                }
            }
        }
        if (empty($loader_path)) {
            if (preg_match("/^\s*$wrong_ext_start\s*=\s*\"?([^\"]+)\"?/i",$l,$bad_start_matches)) {
                if (preg_match("/$loader_name_match/i",$bad_start_matches[1])) {
                    $bad_zend_ext_msg = "Линия для IonCube Loader в файле конфигурации, $php_ini, должна быть первой $ext_start и <b>не</b> $wrong_ext_start.";
    ...
    if ($loader_path === '') {
        $errors[ERROR_INI_ZE_LINE_NOT_FOUND] = "Необходимая zend_extension строка не может быть найден в конфигурации, $php_ini.";
    } elseif (!@file_exists($loader_path)) {
        $errors[ERROR_INI_LOADER_FILE_NOT_FOUND] = "Загруженный файл  $loader_path, перечислены в конфигурационном файле, $php_ini, не существует или не доступен.";
    } elseif (basename($loader_path) == $loader_path) {
        $errors[ERROR_INI_NOT_FULL_PATH] = "Полный путь должен быть указан для файла загрузчика в файле конфигурации, $php_ini.";
    }
...
function scan_inis_for_loader()
{
    ...
    if (empty($sysinfo['PHP_INI'])) {
        $ini_files_not_found = array("Главный INI файл");
    ...
    foreach ($ini_file_list as $f) {
        ...
        if ($shared_server && array_key_exists(ERROR_INI_NOT_FOUND,$ldloc['errors'])) {
            if (false == user_ini_space_path($f)) {
                $ldloc['errors'][ERROR_INI_NOT_FOUND] = "Системный INI файл не может быть найден или прочитан с помощью мастера - вы не можете сделать это на общем сервере.";
            } else {
    ...
    if (!empty($ini_files_not_found)) {
        ...
        $ldloc['errors'][ERROR_INI_NOT_FOUND] = "Следующий ini файл$plural не может быть найден в загрузчике: " . join(',',$ini_files_not_found);
        if (is_restricted_server()) {
            $ldloc['errors'][ERROR_INI_NOT_FOUND] .= "<br> Это может быть из-за ограничений сервера на месте.";
        }
    }
    if (empty($ldloc['location'])) {
        $ldloc['errors'][ERROR_INI_ZE_LINE_NOT_FOUND] = "Необходимая zend_extension строка не может быть найдена в конфигурации.";
    }
    return $ldloc;
}
...
function find_loader($search_directories_if_not_ini = false)
{
    ...
    if (empty($location) && (empty($errors) || $search_directories_if_not_ini)) {
        ...
        if (empty($location)) {
            $errors[ERROR_LOADER_NOT_FOUND] = 'Файл погрузчик не может быть найден в стандартных местах.';
        }
    }
    if (!empty($errors)) {
        return $errors;
...
function runtime_loading_instructions()
{
    ...
    echo '<h4>Инструкция по загрузке</h4>';
    echo '<div class=panel>';
    echo '<p>На вашем общем сервере погрузчик может быть установлен с помощью метода загрузки во время выполнения.';
    echo " (<a href=\"{$default}&amp;manual=1\">Пожалуйста, нажмите здесь, если вы <strong>не</strong> на общем сервере</a>.)</p>";

    if ('.' == extension_dir()) {
        $dirphrase = is_ms_windows()?'folder':'directory';
        echo "Пожалуйста обратите внимание что на вашей системе загрузчик <em>должен</em> быть таким же " . $dirphrase . " как первый закодированный доступный файл";
    }
    echo '<ol>';
...
function runtime_loading_errors()
{
    ...
    $ext_path = extension_dir_path();
    if (false === $ext_path) {
        $errors[ERROR_RUNTIME_EXT_DIR_NOT_FOUND] = "Каталог расширений не может быть найден.";
    } else {
        $expected_file = dirname(__FILE__) . DIRECTORY_SEPARATOR . get_loader_name();
        if (!@file_exists($expected_file)) {
            $errors[ERROR_RUNTIME_LOADER_FILE_NOT_FOUND] = "Файл погрузчик должен был быть в $expected_file но не может быть найден.";
        } else {
...
function loader_download_instructions()
{
    ...
    if (is_ms_windows()) {
        if (is_bool($sysinfo['THREAD_SAFE'])) {
            $download_str = '<li>Скачать один из следующих архивах ОС Windows ' . $sysinfo['PHP_COMPILER'];
            ...
            echo "<p>Пожалуйста, обратите внимание, что версия установки MS Windows подходит как для непосредственной установки на машине Windows, или для 
     загрузки с локального компьютера на сервер.<br>";
            echo 'Архив загрузчика также можно загрузить с <a href="' . LOADERS_PAGE . '" target="loaders">' . LOADERS_PAGE . '</a>.';
        } else {
            echo '<li>Скачать aрхив Windows загрузчиков с  <a href="' . LOADERS_PAGE  . '" target=loaders>здесь</a>. Если PHP построен где потокобезопасность отключена, используйте Windows non-TS Погрузчики.';
        }
    } else {
        list($basename,$multiple_os_versions) = unix_package_name(); 
        if ($basename == "") {
            echo '<li>Скачать ' . $loader['osname'] . ' ' . $loader['arch'] . ' архив загрузчиков  <a href="' . LOADERS_PAGE . '" target="loaders">здесь</a>.';
            echo "<br>Появляется Ваша система будет ${loader['osnamequal']} для ${loader['wordsize']} бит. Если Погрузчики не доступно для данного точной выпуска ${loader['osname']}, Погрузчики, построенные для более ранней версии должен работать. Обратите внимание, что вам может понадобиться установить предыдущую библиотеку совместимости для операционной системы.";
            echo '<br>Если вы не можете найти подходящую погрузчик то, пожалуйста, создайте тикет в <a href="'. SUPPORT_SITE . '">в нашу службу поддержки</a>.';
        } else {
            echo '<li>Скачать один из следующих архивах погрузчиков ' . $loader['osnamequal'] . ' ' . $loader['arch'] . ':'; 
            ...
            echo "<p>Пожалуйста, обратите внимание, что версия установки MS Windows подходит для загрузки из Windows PC на свой ${loader['osname']} сервер.<br>";
            echo "</p>";
            if ($multiple_os_versions && !$exact_match) {
                echo "<p>Обратите внимание, что вам может понадобиться установить предыдущую библиотеку совместимости для  ${loader['osname']}.</p>";
            }
        }
    }
...
function loader_install_instructions($server_type,$loader_dir = '')
{
    ...
    if (SERVER_LOCAL == $server_type) {
        echo "<li>Поместите файлы Loader в <code>$loader_dir</code></li>";
    } else {
        echo "<li>Перенесите файлы Loader на веб-сервер и установите в <code>$loader_dir</code></li>";
    }
    return $loader_dir;
}
...
function zend_extension_instructions($server_type,$loader_dir)
{
    ...
    if (SERVER_SHARED == $server_type && own_php_ini_possible()) {
        ...
        if (@file_exists($php_ini_path)) {
            $edit_line = "<li>Редактировать <code>$php_ini_name</code> в <code>$ini_dir</code> папке";
            if (zend_extension_line_missing($php_ini_path) && @is_writeable($php_ini_path) && @is_writeable($ini_dir)) {
                ...
                if ($fh !== false) {
                    ...
                    fclose($fh);
                    $editing_ini = false;
                    echo "<li>Ваш php.ini файл $php_ini_path был изменен, чтобы включить необходимую строку для ionCube Loader.";
                } else {
                    echo $edit_line;
                }
            } else {
            ...
            $download_ini_file = "<li><a href=\"$base&amp;page=phpconfig&amp;ininame=$php_ini_name&amp;stype=$server_type_code&amp;download=1&amp;prepend=1\">Save this  <code>$php_ini_name</code> file</a> and upload it to <code>$ini_dir</code> (полный путь на сервере).";
            if (@is_writeable($ini_dir)) {
                ...
                if ($fh !== false) {
                    ...
                    fclose($fh); 
                    echo "<li><code>$php_ini_name</code> файл был создан для вас в <code>$ini_dir</code>.";
                } else {
        ...
        if (empty($sysinfo['PHP_INI_DIR'])) {
            echo "<li>Измените файл <code>${sysinfo['PHP_INI']}</code>";
        } else {
            $php_ini_path = find_additional_ioncube_ini();
            if (empty($php_ini_path)) {
                $php_ini_name = ADDITIONAL_INI_FILE_NAME;
                echo "<li><a href=\"$base&amp;page=phpconfig&amp;download=1&amp;newlinesonly=1&amp;ininame=$php_ini_name&amp;stype=$server_type_code\">Сохраните $php_ini_name файл</a> и положить его в ини каталог файлов, <code>${sysinfo['PHP_INI_DIR']}</code>";
                $editing_ini = false;
            } else {
                $php_ini_name = basename($php_ini_path);
                echo "<li>Измените файл <code>$php_ini_path</code>";
            }
        }
    } else {
        echo "<li>Измените системный <code>$php_ini_name</code> файл";
    }
    if ($editing_ini) {
        echo " и <b>до</b> любой другой строки с '$kwd'  убедитесь, что следующая включена:<br>";
        foreach ($zend_extension_lines as $zl) {
            echo "<code>$zl</code><br>";
        }
        if (!empty($php_ini_path)) {
            /*if (zend_extension_line_missing($php_ini_path)) {
                echo "<a>Кроме того, заменить ваш текущий <code>$php_ini_path</code> файл на <a href=\"$base&amp;page=phpconfig&amp;ininame=$php_ini_name&amp;stype=$server_type_code&amp;download=1&amp;prepend=1\">этот новый $php_ini_name файл</a>."; 
            }*/
        }
    }
    echo '</li>';
...
function server_restart_instructions()
{
    ...
    if ($sysinfo['SS']) {
        echo "<li>Перезагрузите  ${sysinfo['SS']} серверное ПО.</li>";
    } else {
        echo "<li>Перезапустите программное обеспечение сервера.</li>";
    }

    echo "<li>Когда сервер перезапущен, <a href=\"$base&amp;page=loader_check\" onclick=\"showOverlay();\">Нажмите сюда, чтобы проверить Loader</a>.</li>";

    if ($sysinfo['SS'] == 'Apache' && !is_ms_windows()) {
        echo '<li>Если установка Loader не удалась, проверьте файл Apache ошибки в журнале для ошибок и увидите наш гид <a target="unix_errors" href="'. UNIX_ERRORS_URL . '">Unix related errors</a>.</li>';
    }
}
...
{
    $base = get_base_address();
    echo "<li><a href=\"$base&amp;page=loader_check\" onclick=\"showOverlay();\">Нажмите сюда, чтобы проверить Loader</a>.</li>";
}

function link_to_php_ini_instructions()
{
    $default = get_default_address();
    echo "<p><a href=\"{$default}&amp;stype=s&amp;ini=1\">Пожалуйста кликните здесь для использования  php.ini мотода</a>.</p>";
}

function php_ini_instruction_list($server_type)
{
    echo '<h4>Инструкции по установке</h4>';
    echo '<div class=panel>';
    echo '<ol>';
...
function php_ini_install_shared($give_preamble = true)
{
    ...
    $default = get_default_address();
    if ($give_preamble) {
        echo "<p>На вашем <strong>расшаренном</strong> сервере, the Loader должен быть установлен используя <code>$php_ini_name</code> конфирурационный файл.";
        echo " (<a href=\"{$default}&amp;manual=1\">Пожалуйста кликните здесь если вы  <strong>не</strong> расшарили сервер</a>.)</p>";
    }

    if (own_php_ini_possible()) {
        echo '<p>С вашей учетной записи хостинга, вы можете быть в состоянии использовать свой собственный файл конфигурации PHP.</p>';
    } else {
        echo "<p>Похоже, что вы не можете установить IonCube Loader с помощью <code>$php_ini_name</code> файла. Ваш сервер провайдера или системный администратор должен иметь возможность выполнять установку для вас. Пожалуйста, обратитесь к ним со следующими инструкциями.</p>";
    }
...
function php_ini_install($server_type_desc = null, $server_type = SERVER_DEDICATED, $required = true)
{
    ...
    echo '<p>';
    if ($server_type_desc) {
        echo "Для <strong>$server_type_desc</strong> сервера ";
    } else {
        echo "Для этого сервера ";
    }

    if ($required) {
        echo "Вам необходимо установить IonCube Loader с помощью <code>$php_ini_name</code> файла конфигурации.";
    } else {
        echo "установкой IonCube Loader помощью <code>$php_ini_name</code> рекомендуемого файла.";
    }
    if ($server_type_desc) {
        echo " (<a href=\"{$default}&amp;manual=1\">Пожалуйста кликните здесь если Вы <strong>не</strong> на $server_type_desc сервере</a>.)";
    }
    echo '</p>';
...
function help_resources($error_list = array())
{
    ...
    if (SERVER_SHARED != $server_type || own_php_ini_possible(true)) {
		$support_info = array ( 
			...
			'subject' 			=> "ionCube Loader проблема установки",
        ...
        $resources[2] = '<form action="' . SUPPORT_SITE . 'lw_index.php' .'" method="POST" id="support-ticket"><a href="" onclick="document.getElementById(\'support-ticket\').submit(); return false;">Поднять в службу поддержки через нашу службу поддержки</a>';
    ...
    if (SERVER_SHARED == $server_type && own_php_ini_possible(true) && !user_ini_space_path($sysinfo['PHP_INI'])) {
        $resources[3] = '<strong>Пожалуйста, свяжитесь с хозяином, который может создать файлы php.ini, чтобы преодолеть систему одному.</strong>';
    }
    return $resources;
}
...
function system_info_archive_page()
{
    ...
    if ($archive_name) {
        ...
        $server_type_code = server_type_code();
        heading();
        echo "<p>Загружаемые архив системной информации не может быть создан.<br> 
            <strong>Пожалуйста, сохраните каждый из следующих, а затем приложите эти файлы в тикете поддержки:</strong></p>"; 
        echo "<ul>";
        ...
        echo "<li><a href=\"$base&amp;page=extra&amp;stype=$server_type_code\" target=\"extra\">дополнительная информация</a></li>";
        echo "<li><a href=\"$self?page=loaderbin\">Loader файл</a></li>";
        echo "</ul>";
        footer(true);
    }
}
...
function support_ticket_information($error_list = array())
{
    ...
    $ticket_strs = array();
    $ticket_strs[] = "Пожалуйста, не удаляйте следующую информацию\r\n";
    $ticket_strs[] = "==============\r\n";
    if (!empty($error_list)) {
        $ticket_strs[] = "[hr]";
        $ticket_strs[] = "ERRORS";
...
function os_arch_string_check($loader_str)
{
    ...
    if (preg_match("/target os:\s*(([^_]+)_([^-]*)-([[:graph:]]*))/i",$loader_str,$os_matches)) {
        ...
        if (strtolower($dirname) != $os_matches[1] && $packed_osname != $os_matches[2]) {
            $errors[ERROR_LOADER_WRONG_OS] = "Вы ошиблись c Loader для вашей операционной системы, ". $loader_info['osname'] . ".";
        } else {
            ...
            if ($loader_info['arch'] != ($ap = required_loader_arch($os_matches[3],$loader_info['oscode'],$loader_wordsize))) {
                $err_str = "Вы ошиблись c Loader для вашей архитектуры машины.";
                $err_str .= " Ваша система " . $loader_info['arch'];
                $err_str .= " но Loader вы используете для " . $ap . ".";
...
function loader_compatibility_test($loader_location)
{
    ...
    if (LOADER_NAME_CHECK) {
        ...
        if ($installed_loader_name != $expected_loader_name) {
            $errors[ERROR_LOADER_UNEXPECTED_NAME] = "Установленная loader (<code>$installed_loader_name</code>) не имеет имя ожидаемого (<code>$expected_loader_name</code>) для вашей системы. Пожалуйста, проверьте, что у вас есть правильный Loader для вашей системы.";
        }
    }
    if (empty($errors) && !is_readable($loader_location)) {
        $execute_error = "Loader  $loader_location как представляется, не читаем.";
        $execute_error .= "<br>Пожалуйста, проверьте, что он существует и доступен для чтения.";
        $execute_error .= "<br>Пожалуйста, проверьте разрешения, содержащий ";
        $execute_error .= (is_ms_windows()?'folder':'directory') . '.';
        if (($sysinfo['SS'] == 'IIS') || !($sysinfo['IS_CGI'] || $sysinfo['IS_CLI'])) {
            $execute_error .= "<br>Также проверьте, что веб-сервер был перезапущен.";
        }
        $execute_error .= ".";
    ...
    if (preg_match("/php version:\s*(.)\.(.)\.(..?)(-ts)?/i",$loader_strs,$version_matches)) {
        if ($version_matches[1] != $phpv['major'] || $version_matches[2]  != $phpv['minor']) {
            ...
            $errors[ERROR_LOADER_PHP_MISMATCH] = "Установленная Loader для PHP $loader_php но ваш сервер работает PHP $server_php.";
        }
        if (is_bool($sysinfo['THREAD_SAFE']) &&  $sysinfo['THREAD_SAFE'] && !is_ms_windows() && !(isset($version_matches[4]) && $version_matches[4] == '-ts')) {
            $errors[ERROR_LOADER_NONTS_PHP_TS] = "Ваш сервер запущен в режиме thread-safe version  PHP, но loader не thread-safe version.";
        } elseif (isset($version_matches[4]) && $version_matches[4] == '-ts' && !(is_bool($sysinfo['THREAD_SAFE']) &&  $sysinfo['THREAD_SAFE'])) {
            $errors[ERROR_LOADER_TS_PHP_NONTS] = "Ваш сервер запущен в режиме non-thread-safe version  PHP но loader не thread-safe version.";
        }
    } elseif (preg_match("/ioncube_loader_.\.._(.)\.(.)\.(..?)(_nonts)?(_amd64)?\.dll/i",$loader_strs,$version_matches)) {
        if (!is_ms_windows()) {
            $errors[ERROR_LOADER_WIN_SERVER_NONWIN] = "У вас есть загрузчик Windows, но ваш сервер не работает на Windows.";
        } else {
            if (isset($version_matches[4]) && $version_matches[4] == '_nonts' && is_bool($sysinfo['THREAD_SAFE']) &&  $sysinfo['THREAD_SAFE']) {
                $errors[ERROR_LOADER_WIN_NONTS_PHP_TS] = "У вас режим non-thread-safe version на Windows loader , но вам нужен режим thread-safe one.";
            } elseif (!(is_bool($sysinfo['THREAD_SAFE']) &&  $sysinfo['THREAD_SAFE']) && !(isset($version_matches[4]) && $version_matches[4] == '_nonts')) {
                $errors[ERROR_LOADER_WIN_TS_PHP_NONTS] = "У вас режим thread-safe version на Windows loader , но вам нужен non-thread-safe one."; 
            }
            if ($version_matches[1] != $phpv['major'] || $version_matches[2]  != $phpv['minor']) {
                $loader_php = $version_matches[1] . "." . $version_matches[2];
                $server_php =  $phpv['major'] . "." .  $phpv['minor'];
                $errors[ERROR_LOADER_WIN_PHP_MISMATCH] = "Установлен loader для PHP $loader_php , но ваш сервер запущен PHP $server_php.";
            }
            ...
            if ($loader_compiler != $sysinfo['PHP_COMPILER']) {
                $errors[ERROR_LOADER_WIN_COMPILER_MISMATCH] = "Ваш loader был построен с использованием $loader_compiler но вам нужен Loader загрузчик построенНый с использованием ${sysinfo['PHP_COMPILER']}.";
            }
        }
    } else {
            $errors[ERROR_LOADER_PHP_VERSION_UNKNOWN] = "PHP версия для погрузчика не может быть определена - пожалуйста, проверьте, что у вас есть действительная ionCube Loader.";
    } 
...
function shared_server()
{
    if (!$rtl_path = runtime_loading()) {
        ...
        echo "<p>ionCube Loader $lv Для PHP $phpv был успешно установлен.</p>";
...
{
    return "Информация, которую вы пытались получить доступ был отключен по соображениям безопасности. Установите заново этот сценарий Мастер погрузчик и попробуйте еще раз.";
}
...
function run()
{
    ...
    (php_sapi_name() == 'cli') && die("Этот сценарий должен быть запущен только веб-сервером.\n");
...
function initial_page()
{
    ...
    <head>
        ...
        function checkNextStep(ajax,expected,continuation) {
            if (ajax.readyState==4 && ajax.status==200)
            {
                ...
                if (ajax.responseText == expected) {
                   ...
                   statusPar.innerHTML = 'Невозможно проверить наличие обновлений<br>скрипта';
        ...
        var loaderVersionCheck = function() {
            statusPar.innerHTML = 'Шаг 4/4: Получение последних версий загрузчиков';
        ...
        var platformCheck = function() {
            statusPar.innerHTML = 'Шаг 3/4: Получение информации платформы';
            var xmlHttp = getXmlHttp();
            xmlHttp.onreadystatechange=function() {
        ...
        var compilerVersionCheck = function() {
            statusPar.innerHTML = 'Шаг 2/4: Получение версии компилятора';
            var xmlHttp = getXmlHttp();
            xmlHttp.onreadystatechange=function() {
        ...
        var startChecks = function() {
            ...
            statusPar.innerHTML = 'Шаг 1/4: Получение версию мастера загрузки';
            var xmlHttp = getXmlHttp();
            xmlHttp.onreadystatechange=function() {
        ...
        function ajaxTimeout(){
           ajax.abort();
           statusPar.innerHTML = 'Скрипт не может достучаться до <br>сервера';
           setTimeout("window.location.href = '$start_page&timeout=1'",1000);
        }
        </script>
    </head>
    ...
    <div id="loading"><script type="text/javascript">document.write('<p>Инициализация<br>ionCube Loader мастера<br><span id="status"></span></p>');</script><p id="noscript">Ваш браузер не поддреживает JavaScript поэтому ionCube Loader Мастер установки не может быть закончен. Этот сценарий может получить самую свежую информацию погрузчик версии с сервера IonCube, когда вы идете к следующей странице.<br>Пожалуйста, выберите один из следующих. <br>Если сценарий зависает перезагрузите сценарий и выберите опцию «НЕТ».<br><br><br><a href="$start_page">ДА - на моем сервере ДЕЙСТВИТЕЛЬНО есть доступ в Интернет.</a><br><br><a href="$start_page&timeout=1">НЕТ - мой сервер не имеет доступа в Интернет</a></p></div>
    <script type="text/javascript">
...
function default_page($loader_extension = LOADER_EXTENSION_NAME)
{
    ...
    foreach (array('self') as $vn) {
        if (empty($$vn)) {
            ...
            error("Невозможно инициализировать ($vn)". ' $_SERVER is: ' . $server_data);
        }
    }
...
{
    echo '<p><strong>По соображениям безопасности мы советуем вам удалить этот файл с вашего сервера где установлен IonCube Loader.</strong></p>';
}

function contact_script_provider_instructions()
{
    echo '<p>Пожалуйста, обратитесь к поставщику скрипта, если у вас возникли проблемы с запуском закодированных файлов.</p>';
}
...
function may_need_to_copy_ini()
{
    ...
    if (ini_same_dir_as_wizard() && $sys['IS_CGI']) {
        $dirphrase = is_ms_windows()?'folder':'directory';
        $ini = ini_file_name();
        echo "<p>Пожалуйста, обратите внимание, что если в кодировке файлов в другой $dirphrase от мастера неудачу, то вы должны попытаться скопировать $ini файл каждого $dirphrase , в котором Вы кодировали файлы.</p>";
    }
}
...
function successful_install_end_instructions($rtl_path = null)
{
    if (empty($rtl_path)) {
        ...
        echo "<p>Метод загрузки во время выполнения установки был использован с пути <code>$rtl_path</code></p>";
    }
...
function loader_major_version_instructions($mv)
{
    if ($mv < LATEST_LOADER_MAJOR_VERSION) {
        echo "<p><strong>Установленная версия Loader не может работать с файлами, созданными самой последней ionCube Encoder.</strong>";
        echo " Вам нужна версия " . LATEST_LOADER_MAJOR_VERSION . " ionCube Loader для запуска таких файлов</p>";
    }
...
function loader_already_installed($rtl = null)
{
    list($lv,$mv,$newer_version) = ioncube_loader_version_information();
    $phpv = php_version_maj_min();
    $php_str = ' для PHP ' . $phpv;
    echo '<div class="success">';
    echo '<h4>Loader Установлен</h4>';
    if ($newer_version) {
        echo '<p>ionCube Loader версии ' . $lv . $php_str . ' <strong>уже установлен</strong> но это старая версия.';
        echo ' Рекомендуется, чтобы Loader был повышен до последней версии, если это возможно.</p>';
        $know_latest_version = is_string($newer_version);
        $is_legacy_loader = loader_major_version_instructions($mv);
        echo '</div>';
        loader_upgrade_instructions($lv,$newer_version);
    } else {
        echo '<p>ionCube Loader версии ' . $lv . $php_str . ' уже установлен и кодированные файлы должны запускаться без проблем.</p>'; 
        echo '</div>';
...
function loader_upgrade_instructions($installed_version,$newer_version)
{
    if ($newer_version) {
        echo '<div class="panel">';
        echo '<h4>Loader - инстукции для обновления</h4>';
        $restart_needed = true;
        $server_type = find_server_type();
        if ($server_type == SERVER_SHARED || $server_type == SERVER_UNKNOWN) {
            ...
            if (!is_string($loader_path) || false === user_ini_space_path($loader_path)) {
                ...
                echo "<p>Обратите внимание, что вам $verb_case нужно вашему системному администратору сделать следующее, чтобы обновить. Веб-сервер необходимо будет перезапущен после того, как файл Loader изменится.</p>";
            }
            $restart_needed = false;
        }
        if (is_string($newer_version)) {
            $version_str = "версияn $newer_version";
        } else {
            $version_str = "новая версия";
        }
        $loader_name =  get_loader_name();
        echo "<p>Обновите вашу версию $installed_version на $version_str  ionCube Loader, пожалуйста, замените существующий файл загрузчика, $loader_name, на
            файл с таким же именем из одного из следующих пакетов:</p>";
        if (is_ms_windows()) {
            $basename = windows_package_name();
        } else {
            list($basename,$multiple_os_versions) = unix_package_name();
        ...
        if ($restart_needed) {
            echo "<p>После замены файла загрузчика перезагрузите веб-сервер.</p>";
        }
        echo '</div>';
    }
}
...
{
    $leg_warn = '<p><strong>Вы находитесь на платформе, на которой Погрузчики IonCube больше не разрабатывается. ';
    $leg_warn .= 'Загрузчики на вашей платформе не могут иметь возможность запускать файлы, полученные от последней IonCube Encoder. ';
    $leg_warn .= 'Пожалуйста, переключиться, если это возможно, к платформе, на которой погрузчики в настоящее время при поддержке. ';
    $leg_warn .= 'Список поддерживаемых в настоящее время платформ показан на нашем <a href="' . LOADERS_PAGE . '" target="loaders">странице Loaders</a>.</strong></p>';
...
function loader_not_installed()
{
    ...
    if ($host_type != SERVER_UNKNOWN && is_array($loader) && !$sysinfo['DEBUG_BUILD']) {
        ...
        if (!empty($errors)) {
            if (count($errors) > 1) {
                $problem_str = "Обратите внимание, что в настоящее время существуют следующие проблемы";
            } else {
                $problem_str = "Обратите внимание, что следующая задача в настоящее время существует";
            }
            echo '<div class="alert">' .$problem_str . ' с установкой IonCube Loader:';
        ...
        if (!empty($warnings)) {
            ...
            echo "Обратите внимание $addword на возможные проблемы $plural:";
            echo make_list($warnings,"ul"); 
            echo '</div>';
        }
    }
    if (!isset($stype)) {
        echo '<p>Для использования файлов, которые были под защитой <a href="' . ENCODER_URL . '" target=encoder>ionCube PHP Encoder</a>, компонент называется IonCube Loader должен быть установлен.</p>';
    }

    if (!is_supported_php_version()) {
        echo '<p>Ваш сервер запущен под PHP версией ' . PHP_VERSION . ' и не подтверждается ionCube Loaders.  Рекомендуется  PHP 4.2 или выше, 
                и PHP 5.1 или выше.</p>';
	} elseif ($latest_supported_php_version = is_after_max_php_version_supported()) {
		echo '<strong>Ваш сервер запущен с версией PHP  ' . PHP_VERSION . ' и в настоящее время не подкреплены никакими ionCube Loaders. <br/>Это может измениться в будущем, если Loader производится для вашей PHP платформы.<br/>В то же время, пожалуйста, понизить PHP до версии ' . $latest_supported_php_version . '.</strong>';
    } elseif ($sysinfo['DEBUG_BUILD']) {
         echo '<p>Ваш сервер в настоящее время работает отладочной сборки PHP. Loader не может быть установлен с построения отладки РНР. Пожалуйста, убедитесь, что PHP реконфигурируется с отладки отключена. Обратите внимание, что отладки сборки PHP не может помочь в отладке сценариев PHP.</p>'; 
    } elseif (!is_array($loader)) {
        if ($loader == ERROR_WINDOWS_64_BIT) {
            echo '<p>Loaders для 64-bit PHP на Windows в настоящее время отсутствуют. Однако, если вы <b>установили и запустили 32-bit PHP</b> соответствующий 32-битный загрузчик Windows должен работать.</p>';
            if ($sysinfo['THREAD_SAFE']) {
                echo '<li>Скачать один из следующих архивах 32-битных ОС Windows x86 загрузчиков:';
            } else {
                echo '<li>Скачать один из следующих архивах 32-битных ОС Windows, не TS x86 загрузчиков:';
            }
            echo make_archive_list(windows_package_name());
        } else {
            echo '<p>Загрузчик IonCube не может быть  доступен для вашего типа системы на данный момент. Тем не менее, если вы создаете <a href="'  . SUPPORT_SITE . '">тикет в поддержкуt</a> нашишите больше советов и информации для оказания помощи. Пожалуйста, включите URL для этого мастера в вашем тикете.</p>';
        }
        ...
        echo '<p>В настоящее время IonCube загрузчик  PHP будет построен с ' . $supported_compiler_string . '. Ваш PHP программного обеспечения был построен с использованием ' . $sysinfo['PHP_COMPILER'] . '. Поддерживаемые сборки PHP можно получить <a href="http://windows.php.net/download/">PHP.net</a>.';
    } else {
        switch ($host_type) {
            case SERVER_SHARED:
                shared_server();
...
function server_selection_form()
{
    ...
    $form = <<<EOT
    <p>Этот мастер даст вам информацию о том, как установить IonCube Loader.</p>
    <p>Пожалуйста, выберите тип веб-сервера, что у вас есть и нажмите кнопку Далее.</p>
    <script type=text/javascript>
        function trim(s) {
        ...
        function input_ok() {
            ...
            if (!s.checked && !d.checked) {
                alert("Пожалуйста, выберите один из типов серверов.");
                return false;
            } else {
                ...
                if (!hostprovider || !hosturl) {
                    alert("Пожалуйста, введите и имя хостинг-провайдер и их URL.");
                    return false;
                }
                if (hostprovider.length < 4) {
                    alert("Хостинг имя поставщика должна быть не менее 4 символов.");
                    return false;
                }
                if (!hosturl.match(/[A-Za-z0-9-_]+\.[A-Za-z0-9-_%&\?\/.=]+/)) {
                    alert("URL-адрес хостинг-провайдер является недействительным.");
                    return false;
                }
                if (hosturl.length < 5) {
                    alert("Хостинг URL поставщик должен быть не менее 5 символов.");
                    return false;
                }
            }
            return true;
    ...
    <form method=GET action=$self>
        ...
        <input type=radio id=shared name=stype value=s onclick="document.getElementById('hostinginfo').style.display = 'block';"><label for=shared>Общий <small>(например, сервер с доступом к FTP только и не имеют доступа к php.ini)</small></label><br>
        <input type=radio id=dedi name=stype value=d onclick="document.getElementById('hostinginfo').style.display = 'block';"><label for=dedi>Выделенный или VPS <small>(сервер с полным доступом root SSH)</small></label><br>
        <div id="hostinginfo" style="display: none">Если вы находитесь на общем или выделеннОм сервере, пожалуйста, дайте своему хостинг-провайдеру этот URL:
            <table>
                <tr><td><label for=hostprovider>Имя Вашего хостинг провайдера</label></td><td><input type=text id="hostprovider" name=hostprovider value="$hostprovider"></td></tr>
                <tr><td><label for=hosturl>Адрес вашего хостинг-провайдера</label></td><td><input type=text id="hosturl" name=hosturl value="$hosturl"></td></tr>
            </table>
        </div>
        <input type=radio id=local name=stype value=l onclick="document.getElementById('hostinginfo').style.display = 'none';"><label for=local>Локальная установка</label>
...
function phpinfo_page()
{
    info_disabled_check();
    if (function_is_disabled('phpinfo')) {
        echo "phpinfo отключена на этом сервере";
    } else {
        @phpinfo();
    }
}
...
function loader_check_page($ext_name = LOADER_EXTENSION_NAME)
{
    ...
    if (extension_loaded($ext_name)) {
        ...
        $php_str = ' для PHP ' . $phpv;
        echo '<div class="success">';
        echo '<h4>Загрузчик успешно установлен</h4>';
        echo '<p>ionCube Loader версии ' . $lv . $php_str . ' <strong>установлен</strong> и закодированные файлы должны успешно работать.';
        if ($newer_version) {
            echo ' Пожалуйста, обратите внимание, однако, что у вас есть старая версия IonCube Loader.</p>';
            $is_legacy_loader = loader_major_version_instructions($mv);
            echo '</div>';
            loader_upgrade_instructions($lv,$newer_version);
        } else {
        ...
        echo '<h4>Загрузчик НЕ установлен</h4>';
        echo '<p>ionCube Loader на данный момент установлено <b>не</b>  успешно.</p>';
	
        if (!is_null($rtl_path)) {
            echo '<p>Загрузчиком была предпринята попытка, но безуспешно.</p>';
            echo '</div>';
...
function ini_loader_errors()
{
    ...
    if (SERVER_SHARED == find_server_type() && !own_php_ini_possible(true)) {
        $errors[ERROR_INI_USER_CANNOT_CREATE] = "Похоже, что вы не сможете создать свой собственный INI файлы на вашем общем сервере.<br><strong>Вам нужно будет обратиться к администратору сервера, чтобы он установил IonCube Loader для вас.</strong>";
    }
    ...
    if (is_string($loader_loc)) {
        if (!shared_and_runtime_loading()) {
            ...
            if (empty($sys['PHP_INI'])) {
                $errors[ERROR_INI_NO_PATH] = 'Нет файла конфигурации PHP по найденному пути (php.ini).';
            } elseif (!@file_exists($sys['PHP_INI'])) {
                $errors[ERROR_INI_NOT_FOUND] = 'Файл конфигурации PHP(' . $sys['PHP_INI'] .') не найден.';
            }
        }
        ...
        if (!empty($fs_location)) {
            ...
            if (!empty($fs_loader_errors)) {
                $errors[ERROR_LOADER_WRONG_GENERAL] = "Файл загрузчик найденный на $fs_location не является правильным выбором для вашей системы.";
            }
        ...
        if (empty($loader_errors)) {
            ...
            if (!empty($ini_loader['location'])) {
                ...
                if (@file_exists($std_ld_path)) {
                    ...
                    $stdloc_loader_errors = array("Файл загрузчика не существует.");
                }
    ...
    if (find_server_type() == SERVER_SHARED)
    {
        if (own_php_ini_possible()) {
            ...
            foreach ($ini_files as $full_ini_loc) {
                $advice = "Файл $full_ini_loc не распознается PHP.";
                $advice .= " Пожалуйста, проверьте, что имя и расположение файла верны.";
                if (!ini_same_dir_as_wizard()) {
                    ...
                    if (!@file_exists($shared_ini_file) && !empty($shared_ini_loc) && $ini_loc_dir != $shared_ini_loc && $ini_dir != $shared_ini_loc) {
                        ...
                        $advice .= " Пожалуйста, попробуйте скопировать <code>$full_ini_loc</code> файл в <code>" . $shared_ini_loc . "</code>.";
                    } else {
                        if (!@file_exists($root_ini_file) && $rootpath != $shared_ini_loc && $full_ini_loc != $rootpath) {
                            $all_ini_locations_used = false;
                            $advice .= " Пожалуйста, попробуйте скопировать <code>$full_ini_loc</code> файл <code>" . $rootpath . "</code>.";
                        } 
                        if (!empty($cgibin_ini_file) && !@file_exists($cgibin_ini_file) && $cgibinpath != $shared_ini_loc && $full_ini_loc != $cgibinpath && $cgibinpath != $rootpath) {
                            $all_ini_locations_used = false;
                            $advice .= "  Пожалуйста, попробуйте скопировать <code>$full_ini_loc</code> файл <code>" . $cgibinpath . "</code>.";
                        }
                        ...
                        if (!@file_exists($here_ini_file) && $herepath != $rootpath && $herepath != $cgibinpath) {
                            $all_ini_locations_used = false;
                            $advice .= " Это может быть необходимо, чтобы скопировать <code>$full_ini_loc</code> файл <code>$herepath</code> и все " . (is_ms_windows()?'folders':'directories') . ' , в котором Вы кодировали файлы';
                        }
                    }
                } else {
                    $all_ini_locations_used = false;
            ...
            if ($all_ini_locations_used) {
                $warnings[] = "<strong>Похоже, что ини файлы не распознаются в любых стандартных местах на вашем веб-пространстве. Пожалуйста, свяжитесь с вашим хостинг-провайдером, чтобы проверить, можно ли создать свой собственный файл PHP INI и где он должен быть.</strong>";
            }
        } else {
            if (own_php_ini_possible(true)) {
                $warnings[] = "Вы не можете создавать свои собственные ини файлы на вашем общем сервере. <br><strong>Возможно, вам придется обратиться к администратору сервера, чтобы он установил IonCube Loader для вас.</strong>";
            }
        }
    } else {
        $loader_dir_pair = correct_loader_wrong_location();
        if (!empty($loader_dir_pair)) {
            $advice = "Правильный загрузчик для вашей системы найден. at <code>${loader_dir_pair['loader']}</code>."; 
            if ($loader_dir_pair['loader'] != $loader_dir_pair['newloc']) {
                $advice .= " Вы можете скопировать загрузчик от <code>${loader_dir_pair['loader']}</code> в <code>${loader_dir_pair['newloc']}</code>.";
            }
            $warnings[] = $advice;
        }
    }
...
function list_loader_errors($errors = array(),$warnings = array(),$suggest_restart = true)
{
    ...
    if (!empty($errors)) {
        $try_again = '<a href="#" onClick="window.location.href=window.location.href">попробуйте еще раз</a>';
	
        echo '<div class="alert">';
        if (count($errors) > 1) {
            echo 'Следующие проблемы были найдены при установке IonCube Loader:';
            $retry_message = "Пожалуйста, исправьте эти ошибки и $try_again.";
        } else {
            echo 'Следующая проблема была обнаружена с установкой IonCube Loader:';
            $retry_message = "Пожалуйста, исправьте эту ошибку и $try_again.";
        }
        ...
        if (!empty($warnings)) {
            echo '<div class="warning">';
            echo 'Пожалуйста, обратите внимание на следующее:';
            echo make_list($warnings,"ul");
            echo '</div>';
        }
    } elseif (!empty($warnings)) {
        echo '<div class="warning">';
        echo 'Существуют следующие потенциальные проблемы:';
        echo make_list($warnings,"ul");
        echo '</div>';
    } elseif ($suggest_restart) {
        if (SERVER_SHARED == find_server_type()) {
            echo "<p>Пожалуйста, обратитесь к администратору сервера по установке IonCube Loader.</p>";
        } else {
            if (selinux_is_enabled()) {
                echo "<p>Похоже, что SELinux включена на сервере.Это может быть решено с помощью команды <code>restorecon [full path to loader file]</code> под root. Если это не решит проблему, пожалуйста, следуйте инструкциям на <a target=\"_blank\" href=\"http://www.cuteshift.com/57/install-ioncube-loader-while-selinux-enabled/\">http://www.cuteshift.com/57/install-ioncube-loader-while-selinux-enabled/</a> для установки IonCube Loader, когда SELinux включен.</p>";
            } elseif (grsecurity_is_enabled()) {
                echo "<p>Похоже, что Grsecurity включена на сервере. Пожалуйста, выполните команду, <code>execstack -c [full path to loader file]</code> и перезагрузите веб-сервер.</p>";
            } else {
                $sysinfo = get_sysinfo();
                $ss = $sysinfo['SS'];
                if (!$sysinfo['CGI_CLI'] || is_ms_windows()) {
                    echo "<p>Пожалуйста, убедитесь, что $ss веб-сервер был перезапущен.</p>";
                } 
            }
        }
    }
    echo '<div>';
    echo $retry_message;
    echo " Вы можете просмотреть следующее для получения дополнительной помощи:";
    echo make_list(help_resources($errors),"ul");
    echo '<a href="' . $default . '">Нажмите здесь, чтобы вернуться к началу мастера Loader</a>.</div>';
}
    ...
    if (!empty($download)) {
        ...
        if (empty($ini_file_name)) {
			...
			if (!preg_match('`^.*\.ini$`',$ini_file_name) || preg_match('`/`',$ini_file_name) || preg_match('`\\\`',$ini_file_name)) {
				die("Незаконное имя файла $ini_file_name");
			}
		}
...
{
    $instr = "Похоже, что вы размещен на сервере Windows, в GoDaddy.<br/>";
    $instr .= "Пожалуйста, измените к хостинг-план Linux на GoDaddy.<br />";
    $instr .=  "Если Вы <a href=\"http://help.godaddy.com/\">свяжетесь с их службой поддержки</a> они должны быть в состоянии вам переключить на сервер Linux.";
...
function GoDaddy_linux_instructions($html_dir)
{
    ...
    $instr = array();
    $instr[] = 'В вашем HTML каталоге, ' . $html_dir . ', создайте подкаталог <b>ioncube</b>.';
    if (@file_exists($ini_path)) {
       $instr[] = "Отредактируйте $php_ini_name в вашем  $html_dir и добавьте следующую строку <b>поверх</b>:<br>" . $zend_extension_line ;
    } else {
        $instr[] = "<a href=\"$base&amp;page=phpconfig&amp;ininame=$php_ini_name&amp;stype=s&amp;download=1&amp;prepend=1\">Сохраните этот $php_ini_name файл</a> и загрузить его на свой HTML каталог, $html_dir";
    }
    $instr[] = 'Скачать <a target="_blank" href="' . IONCUBE_DOWNLOADS_SERVER . '"/ioncube_loaders_lin_x86.zip">Linux ionCube Loaders</a>.';
    $instr[] = 'Распакуйте загрузчики и загрузите их в каталог IonCube, который вы создали ранее.';
    $instr[] = 'Кодированные файлы должны теперь работать.';
...
function GoDaddy_page()
{
    ...
    heading();
        ...
        $inst_str = '<h4>GoDaddy Инструкции по установке</h4>';
        $inst_str .= '<p>Похоже, что вы размещаетесь на GoDaddy (<a target="_blank" href="http://www.godaddy.com/">www.godaddy.com</a>). ';
        $inst_str .= "Если это <b>не</b> так, то пожалуйста <a href=\"$base&amp;page=default&amp;host=ngd\">нажмите сюда, чтобы перейти на главную страницу этого мастера установки</a>.</p>";
        $inst_str .= "<p>Если у вас уже установлен загрузчик то, пожалуйста <a href=\"$base&amp;page=loader_check\" onclick=\"showOverlay();\">Нажмите сюда, чтобы проверить загрузчик</a>.</p>";
...
function error($m)
{
    die("<b>ERROR:</b> <span class=\"error\">$m</span><p>Пожалуйста, помогите нам улучшить этот скрипт, <a href=\"". SUPPORT_SITE . "\">сообщите об этой ошибке</a> и включети ссылку URL на скрипт чтобы его проверить.");
}
...
function heading()
{
    ...
    <html>
    <head>
        <title>ionCube Loader установщик</title>
        <link rel="stylesheet" type="text/css" href="$self?page=css">
        <script type="text/javascript">
    ...
    <div id="overlay">
        <div id="inner_overlay">Проверка конфигурации сервера<br>Пожалуйста подождите</div>
    </div>
    <div id="header">
        <img src="?page=logo" alt="ionCube logo">
    </div>
	<div id="important">
	<h3 class="important">ВАЖНО: Убедитесь, что папка с этим скриптом будет удалена если она больше не понадобится.</h3>
	</div>
    <div id="main">
    <h2>ionCube Loader установщик</h2>
EOT;
}
...
function footer($update_info = null)
{
    ...
    { 
        ...
        ldel.innerHTML = '<p>Получение версии установщика<br>Пожалуйста, подождите</p>';
        ...
        xmlhttp.onreadystatechange=function()
        {
            ...
            if (xmlhttp.readyState==4 && xmlhttp.status==200)
            {
                ...
                buttons = '';
                if (wizardversion == '1') {
                    msg = 'У вас сейчас текущую версию <br> IonCube Loader'; 
                } else if (wizardversion != '0') {
                    msg = 'Новая версия, ' + wizardversion + ', из мастер-загрузчика доступна';
                    buttons = '<button onclick="document.getElementById(\'loading\').style.display=\'none\'; window.open(\'$loader_wizard_loc\'); return false">Получить новую версию</button> &nbsp;'; 
                } else {
                    msg = 'Информация о версии не может быть получен от <br>сервер IonCube';
                }
                buttons += '<button onclick="document.getElementById(\'loading\').style.display=\'none\'; return false">Закрыть окно</button>'; 
    ...
    function ajaxTimeout(){
       xmlhttp.abort();
       msg = 'Информация о версии не может быть получен от <br>сервер IonCube';
       button = '<button onclick="document.getElementById(\'loading\').style.display=\'none\'; return false">Закрыть окно</button>';
       var ldel = document.getElementById('loading');
...
EOT;
    ...
    if ($update_info === null) {
        $wizard_version_string .= '<a target="_blank" href="' . $loader_wizard_loc . '" onclick="version_check();return false;">проверить наличие новой версии</a>';
    } else if ($update_info !== false) {
        $wizard_version_string .= '<a href="' . LOADERS_PAGE .'" target="_blank">скачать версию ' . $update_info . '</a>';
    } else {
        $wizard_version_string .=  "текущая";
    }
    $wizard_version_string .= ')'; 
    ...
    echo " | <a href=\"$default\" onclick=\"showOverlay();\">Начать загрузку</a>";
    echo " | <a href=\"$base&amp;page=loader_check\" onclick=\"showOverlay();\">Тест загрузки</a>";
    echo ' | <a href="' . LOADERS_PAGE . '" target="loaders">загрузчики</a>';
