    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <?if($errors):?>
                ...
                <?endforeach?>
            <?endif?>
            <?=Form::open('admin/widgets/wslider/', array('enctype' => 'multipart/form-data', "id" => "form", "role"=> "form", "class"=> "form-horizontal"))?>
            <div class="container-fluid" id="widget">
                <div class="row bg-success">
                    <div class="col-sm-3"><i class="glyphicon glyphicon-asterisk"></i>&nbsp;<span class="btn"><?=$text['entry_size']?></span></div>
                    <div class="col-sm-2"><span class="btn"><?=$text['entry_advert']?></span></div>
                ...
                <?foreach ($widgets as $widget) :?>
                    <div class="row" id="widget-row<?=$widget_row?>"><br/>
                        <div class="col-sm-3">
                            ...
                            <?=Form::input("wslider[".$widget_row."][width]", isset($widget) ? $widget['width'] : "", array('class'=> 'form-control', 'style'=>'width:60px'))?>
                            <?=Form::input("x", "x", array('class'=> 'form-control', 'style'=>'width:35px','disabled'=>'disabled'))?>
                            <?=Form::input("wslider[".$widget_row."][height]", isset($widget) ? $widget['height'] : "", array('class'=> 'form-control', 'style'=>'width:60px'))?>
                            </div>
                        </div>
                        <div class="col-sm-2">
                            <select class="form-control" name="wslider[<?=$widget_row?>][advert_id]">
                                <?php foreach ($adverts as $advert) { ?>
                                    <?if ($advert->id == $widget['advert_id']):?>
                                        <option value="<?=$advert->id?>" selected="selected"><?=$advert->name?></option>
                                    <?else:?>
                        ...
                        <div class="col-sm-2">
                            <select class="form-control" name="wslider[<?=$widget_row?>][layout_id]">
                        ...
                        <div class="col-sm-1">
                            <select class="form-control" name="wslider[<?=$widget_row?>][position]">
                        ...
                        <div class="col-sm-1">
                            <?=Form::checkbox("wslider[".$widget_row."][status]", 1, isset($widget['status']) ? true : false, array('class'=> 'form-control', 'style'=>'width:30px'))?>
                        </div>
                        <div class="col-sm-1">
                            <?=Form::input("wslider[".$widget_row."][sort_order]", isset($widget) ? $widget['sort_order'] : "", array('class'=> 'form-control', 'style'=>'width:50px'))?>
                        </div>
                        <div class="col-sm-1">
                            <a class="btn btn-primary" data-toggle="tooltip" title="<?=$text['button_remove']?>" onclick="$('#widget-row<?=$widget_row?>').remove();" ><i class="glyphicon glyphicon-minus-sign"></i></a></div>
                    </div>
                    <?php $widget_row++; ?>
                <?endforeach?>
...
$('#widget').on('click', '#addWidget',function () {
    html  = '<div class="row" id="widget-row' + widget_row + '" ><br/>';
        html += '<div class="col-sm-3"><div class="input-group"><input class="form-control" style="width:60px" type="text" name="wslider[' + widget_row + '][width]" value="600">';
            html +='<input class="form-control" style="width:35px" type="text" name="x" value="x" disabled="disabled" />';
        html += '<input class="form-control" style="width:60px" type="text" name="wslider[' + widget_row + '][height]" value="250" /></div></div>';
        html += '<div class="col-sm-2"><select class="form-control" name="wslider['+ widget_row +'][advert_id]">';
            <?foreach ($adverts as $advert):?>
            html += '<option value="<?=$advert->id?>"><?=addslashes($advert->name)?></option>';
            <?endforeach?>
        html += '</select></div>';
        html += '<div class="col-sm-2"><select class="form-control" name="wslider[' + widget_row + '][layout_id]">';
        <?foreach ($layouts as $layout):?>
        html += '<option value="<?=$layout->id?>"><?=addslashes($layout->name)?></option>';
        <?endforeach?>
        html += '</select></div>';
        html += '<div class="col-sm-1"><select class="form-control" name="wslider['+ widget_row +'][position]">';
        ...
        html += '</select></div>';
        html += '<div class="col-sm-1"><input class="form-control" style="width:30px" type="checkbox" name="wslider[' + widget_row + '][status]" value="1" checked="checked"></div>';
        html += '<div class="col-sm-1"><input class="form-control" type="text"    name="wslider[' + widget_row + '][sort_order]" value="0" style="width:50px" ></div>';
