    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <?if($errors):?>
                ...
                <?endforeach?>
            <?endif?>
            <?=Form::open('admin/widgets/wcategory/', array('enctype' => 'multipart/form-data', "id" => "form", "role"=> "form", "class"=> "form-horizontal"))?>
                ...
                <?php $widget_row = 0; ?>
                    <?foreach ($widgets as $widget) :?>
                        <div class="row" id="widget-row<?=$widget_row?>"><br/>
                            <div class="col-sm-4">
                                <select class="form-control" name="wcategory[<?=$widget_row?>][layout_id]">
                            ...
                            <div class="col-sm-4">
                                <select class="form-control" name="wcategory[<?=$widget_row?>][position]">
                            ...
                            <div class="col-sm-2"><?=Form::checkbox("wcategory[".$widget_row."][status]", 1, isset($widget['status']) ? true : false, array('class'=> 'form-control', 'style'=>'width:30px'))?></div>
                            <div class="col-sm-1"><?=Form::input("wcategory[".$widget_row."][sort_order]", isset($widget) ? $widget['sort_order'] : "", array('class'=> 'form-control', 'style'=>'width:50px'))?></div>
...
$('#widget').on('click', '#addWidget',function () {
    ...
    html += '<div class="col-sm-4"><select class="form-control" name="wcategory[' + widget_row + '][layout_id]">';
    <?foreach ($layouts as $layout):?>
    html += '<option value="<?=$layout->id?>"><?=addslashes($layout->name)?></option>';
    <?endforeach?>
    html += '</select></div>';
    html += '<div class="col-sm-4"><select class="form-control" name="wcategory['+ widget_row +'][position]">';
    ...
    html += '</select></div>';
    html += '<div class="col-sm-2"><input class="form-control" style="width:30px" type="checkbox" name="wcategory[' + widget_row + '][status]" value="1" checked="checked"></div>';
    html += '<div class="col-sm-1"><input class="form-control" type="text"    name="wcategory[' + widget_row + '][sort_order]" value="0" style="width:50px" ></div>';
