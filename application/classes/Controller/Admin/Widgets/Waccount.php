...
class Controller_Admin_Widgets_Waccount extends Controller_Admin_Widgets {
    public function before() {
        parent::before();
        
        //загрузка языкового файла
        i18n::lang('admin/'.$this->language_folder.'/widgets/waccount');
       
        // Вывод в шаблон
        $this->template->page_title = __('heading_title');
        $this->template->page_url[] = HTML::anchor('admin/widgets/waccount', $this->template->page_title);
    }
    ...
    public function action_index() {
        ...
        if(isset($_POST["waccount"]) && $this->validateModify()){
    	...
    	$widget = ORM::factory('Admin_Extension')->where("key","=","waccount")->find();
    	if( ! $widget->loaded())
    	{
    		$widget = array();	
    	}
        ...
        $content = View::factory($this->template_admin.'widgets/w_account')
