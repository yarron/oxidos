...
class Controller_Index_Widgets_Contentbottom extends Controller_Index {
    ...
    public function action_index() {
			...
			if ($widgets) {
				foreach ($widgets as $widget) {
				    if(!isset($widget['status'])) 
                        ...
                        $status = 1;
					if ($widget['layout_id'] == $layout_id && $widget['position'] == 'content_bottom' && $status) {
						$widget_data[] = array(
							'code'       => $extension['key'],
							'setting'    => $widget,
        ...
        $this->template = View::factory($this->template_index.'widgets/content_bottom')
            ->bind('content_bottom',          $data_widgets);
