...
class Controller_Index_Widgets_Columnright extends Controller_Index {
    ...
    public function action_index() {
			...
			if ($widgets) {
				foreach ($widgets as $widget) {
				    if(!isset($widget['status'])) 
                        ...
                        $status = 1;
					if ($widget['layout_id'] == $layout_id && $widget['position'] == 'column_right' && $status) {
						$widget_data[] = array(
							'code'       => $extension['key'],
							'setting'    => $widget,
        ...
        $this->template = View::factory($this->template_index.'widgets/column_right')
            ->bind('column_right',          $data_widgets);
    }
}